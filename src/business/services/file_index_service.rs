...
impl FileIndexService {
    ...
    pub async fn add_or_update_entry(&self, entry: file_index::ActiveModel) -> Result<file_index::Model, DbErr> {
        file_index::Entity::insert(entry)
            .on_conflict(
                sea_orm::sea_query::OnConflict::columns(vec![file_index::Column::UserId, file_index::Column::Path])
                    .update_columns(vec![
                    ...
                    .value(file_index::Column::RowDeletedAt, Option::<chrono::NaiveDateTime>::None)
                    .value(file_index::Column::FileTrashedAt, Option::<chrono::NaiveDateTime>::None)
    ...
    pub async fn upsert_file(&self, user_id: &str, logical_path: &str, info: &crate::vfs::VfsFileInfo) -> Result<file_index::Model, DbErr> {
        let parent = std::path::Path::new(logical_path).parent().map(|p| p.to_string_lossy().to_string()).unwrap_or_else(|| "/".to_string());
    ...
    pub async fn list_files(&self, user_id: &str, parent_path: &str) -> Result<Vec<file_index::Model>, DbErr> {
    ...
    pub fn list_files_stream(&self, user_id: String, parent_path: String) -> impl futures::Stream<Item = Result<file_index::Model, DbErr>> + 'static {
        let db = Arc::clone(&self.db);
        futures::stream::unfold((user_id, parent_path, 1u64, true), move |(uid, pp, page, has_more)| {
            let db = Arc::clone(&db);
            async move {
                if !has_more {
                    return None;
                ...
                match res {
                    Ok(items) => {
                        ...
                        let results: Vec<Result<file_index::Model, DbErr>> = items.into_iter().map(Ok).collect();
                        Some((futures::stream::iter(results), next_state))
                    }
                    Err(e) => Some((futures::stream::iter(vec![Err(e)]), (uid, pp, page, false))),
                }
            }
        })
        .flatten()
    ...
    pub async fn list_files_paginated(&self, user_id: &str, parent_path: &str, page: i64, page_size: i64) -> Result<(Vec<file_index::Model>, i64), DbErr> {
    ...
    pub async fn list_files_paginated_with_sort(&self, user_id: &str, parent_path: &str, params: VfsPaginationParams<'_>) -> Result<(Vec<file_index::Model>, i64), DbErr> {
        ...
        {
            let kw_str: &str = kw;
            query = query.filter(file_index::Column::Name.contains(kw_str).or(file_index::Column::Path.contains(kw_str)));
        }
        // Sorting
        ...
        query = if is_desc { query.order_by_desc(sort_col) } else { query.order_by_asc(sort_col) };
        let files = query.limit(params.page_size as u64).offset(offset as u64).all(&*self.db).await?;
        let total = self.count_files_with_filter(user_id, parent_path, params.keyword).await?;
        Ok((files, total))
    }
    /// Count files (with search filter)
    async fn count_files_with_filter(&self, user_id: &str, parent_path: &str, keyword: Option<&str>) -> Result<i64, DbErr> {
        let mut query = file_index::Entity::find()
            .filter(file_index::Column::UserId.eq(user_id))
            .filter(file_index::Column::ParentPath.eq(parent_path))
            .filter(file_index::Column::RowDeletedAt.is_null())
    ...
    pub async fn list_files_recursive(&self, user_id: &str, parent_path: &str) -> Result<Vec<file_index::Model>, DbErr> {
        let mut query = file_index::Entity::find().filter(file_index::Column::UserId.eq(user_id)).filter(file_index::Column::RowDeletedAt.is_null()).filter(file_index::Column::FileTrashedAt.is_null());
        if parent_path != "/" {
            let prefix = format!("{}/", parent_path.trim_end_matches('/'));
            query = query.filter(file_index::Column::Path.eq(parent_path).or(file_index::Column::Path.starts_with(prefix)));
        }
        query.all(&*self.db).await
    }
    /// Search files
    pub async fn search_files(&self, user_id: &str, query: &str) -> Result<Vec<file_index::Model>, DbErr> {
        file_index::Entity::find()
            .filter(file_index::Column::UserId.eq(user_id))
    ...
    pub async fn search_files_paginated(&self, user_id: &str, query: &str, page: i64, page_size: i64) -> Result<(Vec<file_index::Model>, i64), DbErr> {
        let offset = (page - 1) * page_size;
        let files = file_index::Entity::find()
            .filter(file_index::Column::UserId.eq(user_id))
            .filter(file_index::Column::Name.contains(query))
    ...
    pub async fn get_file_metadata(&self, user_id: &str, path: &str) -> Result<Option<file_index::Model>, DbErr> {
        file_index::Entity::find()
            .filter(file_index::Column::UserId.eq(user_id))
    ...
    pub async fn get_total_size(&self, user_id: &str, path_prefix: &str) -> Result<i64, DbErr> {
        let condition = file_index::Column::Path.eq(path_prefix).or(file_index::Column::Path.starts_with(format!("{}/", path_prefix)));
    ...
    pub async fn trash_file(&self, user_id: &str, old_path: &str, new_path: &str) -> Result<(), DbErr> {
        let now = chrono::Utc::now();
        let src_prefix = format!("{}/", old_path);
        let txn = self.db.begin().await?;
        // Get top-level node name
        let new_name = if let Some(name) = new_path.split('/').next_back() { name.to_string() } else { String::new() };
        ...
        let condition = file_index::Column::UserId.eq(user_id).and(file_index::Column::Path.like(format!("{}%", src_prefix)));
        ...
        file_index::Entity::update_many()
            .filter(condition)
            .col_expr(file_index::Column::OriginalPath, Expr::col(file_index::Column::Path).into())
            .col_expr(file_index::Column::Path, Expr::cust_with_exprs("REPLACE(path, $1, $2)", vec![old_path.into(), new_path.into()]))
            .col_expr(file_index::Column::ParentPath, Expr::cust_with_exprs("REPLACE(parent_path, $1, $2)", vec![old_path.into(), new_path.into()]))
            .col_expr(file_index::Column::FileTrashedAt, Expr::value(now))
            .col_expr(file_index::Column::RowUpdatedAt, Expr::value(now))
            .exec(&txn)
            .await?;
    ...
    pub async fn restore_file(&self, user_id: &str, trash_path: &str, restored_path: &str) -> Result<(), DbErr> {
        let now = chrono::Utc::now();
        let trash_prefix = format!("{}/", trash_path);
        let txn = self.db.begin().await?;
        // Get top-level node name
        let new_name = if let Some(name) = restored_path.split('/').next_back() { name.to_string() } else { String::new() };
        // Update top-level node
        file_index::Entity::update_many()
            .filter(file_index::Column::UserId.eq(user_id))
            ...
            .col_expr(file_index::Column::Name, Expr::value(new_name))
            .col_expr(file_index::Column::FileTrashedAt, Expr::value(Option::<chrono::DateTime<chrono::FixedOffset>>::None))
            .col_expr(file_index::Column::OriginalPath, Expr::value(Option::<String>::None))
            .col_expr(file_index::Column::RowUpdatedAt, Expr::value(now))
            .exec(&txn)
            .await?;
        // Recursively process children
        let condition = file_index::Column::UserId.eq(user_id).and(file_index::Column::Path.like(format!("{}%", trash_prefix)));
        file_index::Entity::update_many()
            .filter(condition)
            .col_expr(file_index::Column::Path, Expr::cust_with_exprs("REPLACE(path, $1, $2)", vec![trash_path.into(), restored_path.into()]))
            .col_expr(file_index::Column::ParentPath, Expr::cust_with_exprs("REPLACE(parent_path, $1, $2)", vec![trash_path.into(), restored_path.into()]))
            .col_expr(file_index::Column::FileTrashedAt, Expr::value(Option::<chrono::DateTime<chrono::FixedOffset>>::None))
            .col_expr(file_index::Column::OriginalPath, Expr::value(Option::<String>::None))
            .col_expr(file_index::Column::RowUpdatedAt, Expr::value(now))
            .exec(&txn)
    ...
    pub async fn delete_file(&self, user_id: &str, path: &str) -> Result<(), DbErr> {
        ...
        file_index::Entity::delete_many()
            .filter(file_index::Column::UserId.eq(user_id))
            .filter(file_index::Column::Path.eq(path).or(file_index::Column::Path.starts_with(&prefix)))
            .exec(&txn)
            .await?;
        txn.commit().await?;
        Ok(())
    ...
    /// Move/rename file
    pub async fn move_file(&self, user_id: &str, src_path: &str, dst_path: &str) -> Result<(), DbErr> {
        let now = chrono::Utc::now();
        let src_prefix = format!("{}/", src_path);
        let txn = self.db.begin().await?;
        // Resolve new metadata
        let new_name = if let Some(name) = dst_path.split('/').next_back() { name.to_string() } else { String::new() };
        let new_parent = if let Some((parent, _)) = dst_path.rsplit_once('/') { parent.to_string() } else { String::new() };
        let new_parent = if new_parent.is_empty() { "/".to_string() } else { new_parent };
        // Update top-level node
        ...
        let mut update_many = file_index::Entity::update_many()
            .filter(condition)
            .col_expr(file_index::Column::Path, Expr::cust_with_exprs("REPLACE(path, $1, $2)", vec![src_path.into(), dst_path.into()]))
            .col_expr(file_index::Column::ParentPath, Expr::cust_with_exprs("REPLACE(parent_path, $1, $2)", vec![src_path.into(), dst_path.into()]))
            .col_expr(file_index::Column::RowUpdatedAt, Expr::value(now));
        if !dst_path.starts_with("/.recycle_bin") {
    ...
    /// Set favorite color
    pub async fn set_favorite_color(&self, user_id: &str, path: &str, color: i32) -> Result<(), DbErr> {
        file_index::Entity::update_many()
    ...
    pub async fn list_favorites(&self, user_id: &str, color_filter: Option<i32>) -> Result<Vec<file_index::Model>, DbErr> {
        let mut query = file_index::Entity::find()
            .filter(file_index::Column::UserId.eq(user_id))
    ...
    #[allow(clippy::manual_unwrap_or)]
    pub async fn list_favorites_paginated(&self, user_id: &str, params: VfsPaginationParams<'_>, color_filter: Option<i32>) -> Result<(Vec<file_index::Model>, i64), DbErr> {
        let offset = (params.page - 1) * params.page_size;
        let mut query = file_index::Entity::find()
            .filter(file_index::Column::UserId.eq(user_id))
            .filter(file_index::Column::FavoriteColor.gt(0))
        ...
        query = if is_desc { query.order_by_desc(sort_col) } else { query.order_by_asc(sort_col) };
        let files = query.limit(params.page_size as u64).offset(offset as u64).all(&*self.db).await?;
        let total = self.count_favorites_with_filter(user_id, params.keyword, color_filter).await?;
        Ok((files, total))
    }
    /// Count favorite items (with search and color filter)
    async fn count_favorites_with_filter(&self, user_id: &str, keyword: Option<&str>, color_filter: Option<i32>) -> Result<i64, DbErr> {
        let mut query = file_index::Entity::find()
            .filter(file_index::Column::UserId.eq(user_id))
            .filter(file_index::Column::FavoriteColor.gt(0))
            .filter(file_index::Column::FileTrashedAt.is_null())
    ...
    pub async fn list_trash_paginated(&self, user_id: &str, page: i64, page_size: i64) -> Result<(Vec<file_index::Model>, i64), DbErr> {
        let offset = (page - 1) * page_size;
        let files = file_index::Entity::find()
            .filter(file_index::Column::UserId.eq(user_id))
            .filter(file_index::Column::FileTrashedAt.is_not_null())
    ...
    #[allow(clippy::manual_unwrap_or)]
    pub async fn list_trash_paginated_with_sort(&self, user_id: &str, params: VfsPaginationParams<'_>) -> Result<(Vec<file_index::Model>, i64), DbErr> {
        let offset = (params.page - 1) * params.page_size;
        let mut query = file_index::Entity::find()
            .filter(file_index::Column::UserId.eq(user_id))
            .filter(file_index::Column::FileTrashedAt.is_not_null())
        ...
        {
            let kw_str: &str = kw;
            query = query.filter(file_index::Column::Name.contains(kw_str).or(file_index::Column::Path.contains(kw_str)).or(file_index::Column::OriginalPath.contains(kw_str)));
        }
        // Sorting
        let sort_col = match params.sort_by {
        ...
        query = if is_desc { query.order_by_desc(sort_col) } else { query.order_by_asc(sort_col) };
        let files = query.limit(params.page_size as u64).offset(offset as u64).all(&*self.db).await?;
        let total = self.count_trash_files_with_filter(user_id, params.keyword).await?;
        Ok((files, total))
    }
    /// Count trash files (with search filter)
    async fn count_trash_files_with_filter(&self, user_id: &str, keyword: Option<&str>) -> Result<i64, DbErr> {
        let mut query = file_index::Entity::find()
            .filter(file_index::Column::UserId.eq(user_id))
            .filter(file_index::Column::FileTrashedAt.is_not_null())
            .filter(file_index::Column::ParentPath.eq("/.recycle_bin"))
    ...
    pub async fn get_active_shares_for_files(&self, file_ids: &[String]) -> Result<std::collections::HashMap<String, (bool, bool)>, DbErr> {
        ...
        let shares = file_share::Entity::find().filter(file_share::Column::FileIndexId.is_in(file_ids)).all(&*self.db).await?;
    ...
    pub async fn sync_trash_share_status(&self, user_id: &str, file_index_id: &str) -> Result<(), DbErr> {
        use crate::business::entities::file_share;
    ...
    pub async fn update_storage_id(&self, user_id: &str, path: &str, target_storage_id: &str) -> Result<(), DbErr> {
        file_index::Entity::update_many()
            .col_expr(file_index::Column::StorageId, Expr::value(target_storage_id))
            .filter(file_index::Column::UserId.eq(user_id))
            .filter(file_index::Column::Path.eq(path))
            .filter(file_index::Column::RowDeletedAt.is_null())
            .exec(&*self.db)
    ...
    pub async fn sync_directory_items(&self, _user_id: &str, _parent_path: &str, items: Vec<file_index::ActiveModel>) -> Result<(), DbErr> {
        if items.is_empty() {
            return Ok(());
        }
    ...
    pub async fn find_expired_trash(&self, retention_days: u32) -> Result<Vec<file_index::Model>, DbErr> {
        if retention_days == 0 {
            return Ok(vec![]);
        }
    ...
    pub async fn sync_directory_optimized(&self, user_id: &str, parent_path: &str, items: Vec<file_index::ActiveModel>) -> Result<(), DbErr> {
        let parent_path = if parent_path == "/" { "/" } else { parent_path.trim_end_matches('/') };
        if items.is_empty() {
            // If physical side is empty, clear all index records in this directory
            file_index::Entity::delete_many().filter(file_index::Column::UserId.eq(user_id)).filter(file_index::Column::ParentPath.eq(parent_path)).exec(&*self.db).await?;
            return Ok(());
        }
            ...
            file_index::Entity::insert_many(chunk.to_vec())
                .on_conflict(
                    sea_orm::sea_query::OnConflict::columns(vec![file_index::Column::UserId, file_index::Column::Path])
                        ...
                        .value(file_index::Column::RowDeletedAt, Expr::value(Option::<chrono::DateTime<chrono::FixedOffset>>::None))
                        .to_owned(),
                )
                .exec(&txn)
                .await?;
    ...
    pub async fn sync_directory_complete(&self, user_id: &str, parent_path: &str, physical_items: Vec<file_index::ActiveModel>) -> Result<(), DbErr> {
        self.sync_directory_optimized(user_id, parent_path, physical_items).await
    }
}
