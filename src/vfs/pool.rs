...
impl VfsStorage for VfsPool {
    ...
    async fn list(&self, path: &str) -> VfsResult<Vec<VfsFileInfo>> {
        ...
        let list_path = if !path.is_empty() && !path.ends_with('/') { format!("{}/", path) } else { path.to_string() };
        ...
        while let Some(entry_res) = lister.next().await {
            ...
            let (is_dir, size, modified) = if meta.last_modified().is_none() || (meta.is_file() && meta.content_length() == 0) {
                match self.primary.stat(entry.path()).await {
                    Ok(st) => (st.is_dir(), st.content_length(), st.last_modified().map(|t| std::time::SystemTime::from(t).into())),
                    Err(_) => (meta.is_dir(), meta.content_length(), meta.last_modified().map(|t| std::time::SystemTime::from(t).into())),
                }
            } else {
                (meta.is_dir(), meta.content_length(), meta.last_modified().map(|t| std::time::SystemTime::from(t).into()))
            };
    ...
    fn list_stream(&self, path: &str) -> BoxStream<'static, VfsResult<VfsFileInfo>> {
        ...
        let path_clone = if !path.is_empty() && !path.ends_with('/') { format!("{}/", path) } else { path.to_string() };
        Box::pin(futures::stream::once(async move { primary_for_lister.lister(&path_clone).await }).flat_map(move |lister_res| {
            ...
            match lister_res {
                ...
                Err(e) => Box::pin(futures::stream::once(async move { Err(crate::vfs::error::VfsError::from(e)) })) as BoxStream<'static, VfsResult<VfsFileInfo>>,
            }
        }))
    }
    ...
    async fn read_stream(&self, path: &str) -> VfsResult<(std::pin::Pin<Box<dyn futures::Stream<Item = VfsResult<Bytes>> + Send + Sync>>, VfsFileInfo)> {
        self.read_stream_range(path, 0..u64::MAX).await
    }
    async fn read_stream_range(&self, path: &str, range: std::ops::Range<u64>) -> VfsResult<(std::pin::Pin<Box<dyn futures::Stream<Item = VfsResult<Bytes>> + Send + Sync>>, VfsFileInfo)> {
        use futures::StreamExt;
        let info = self.stat(path).await?;
    ...
    async fn write_stream(&self, path: &str, mut stream: BoxStream<'static, VfsResult<Bytes>>) -> VfsResult<VfsFileInfo> {
        use futures::StreamExt;
    ...
    async fn stat(&self, path: &str) -> VfsResult<VfsFileInfo> {
        let meta = self.primary.stat(path).await.map_err(crate::vfs::error::VfsError::from)?;
        ...
        Ok(VfsFileInfo {
            ...
            modified: meta.last_modified().map(|t| std::time::SystemTime::from(t).into()),
            favorite_color: 0,
            has_active_share: None,
            has_active_direct: None,
            trashed_at: None,
    ...
    async fn read_range(&self, path: &str, start: u64, end: u64) -> VfsResult<(Bytes, VfsFileInfo)> {
        let data = self.primary.read_with(path).range(start..end).await?.to_bytes();
        let info = self.stat(path).await?;
        Ok((data, info))
    }
    async fn write_at(&self, _path: &str, _offset: u64, _data: Bytes) -> VfsResult<VfsFileInfo> {
        Err(crate::vfs::error::VfsError::Internal("Write at not supported in Pool".to_string()))
    }
    async fn set_times(&self, path: &str, _atime: Option<u64>, _mtime: Option<u64>) -> VfsResult<VfsFileInfo> {
        self.stat(path).await
    }
    ...
    async fn batch_remove(&self, paths: &[String]) -> VfsResult<VfsBatchResult> {
        ...
        for path in paths {
            match self.delete(path).await {
                ...
                Err(e) => failed.push(crate::vfs::types::VfsBatchError { path: path.as_str().into(), error: e.to_string().into() }),
            }
        }
    ...
    async fn batch_move(&self, src_paths: &[String], dst_dir: &str) -> VfsResult<VfsBatchResult> {
        ...
        for src in src_paths {
            ...
            match self.move_file(src, &dst).await {
                ...
                Err(e) => failed.push(crate::vfs::types::VfsBatchError { path: src.as_str().into(), error: e.to_string().into() }),
            }
        }
        Ok(VfsBatchResult { success, failed })
    }
    async fn compress(&self, source_path: &str, target_path: &str, user_id: &str, password: Option<&str>, encrypt_filenames: bool) -> VfsResult<VfsFileInfo> {
        ...
        compress_compat(self, source_path, target_path, user_id, password, encrypt_filenames).await
    }
    ...
    async fn create_temp_file_for_upload(&self) -> VfsResult<(std::path::PathBuf, crate::utils::VfsTempFileGuard)> {
        Err(crate::vfs::error::VfsError::Internal("Temp file not supported in Pool".to_string()))
    }
    // Business Extensions ===
    fn get_recursive_size(&self, _path: &str) -> futures::future::BoxFuture<'static, VfsResult<i64>> {
        Box::pin(async move { Err(crate::vfs::error::VfsError::Internal("Not implemented in Pool".to_string())) })
    }
    ...
    async fn migrate_storage(&self, path: &str, _target_storage_id: &str) -> VfsResult<VfsFileInfo> {
