...
pub trait VfsStorage: Send + Sync {
    ...
    async fn search_files_paginated(
        ...
        let filtered: Vec<VfsFileInfo> = all
            .into_iter()
            .filter(|e| e.name.to_lowercase().contains(&keyword_lower))
            .collect();
            ...
            && !kw.is_empty()
        {
            ...
            all.retain(|e| {
                e.name.to_lowercase().contains(&kw_lower)
                    ...
                    || e.original_path
                        .as_ref()
                        .is_some_and(|p| p.to_lowercase().contains(&kw_lower))
            });
        }
        // Sorting
        if let Some(field) = params.sort_by {
            ...
            all.sort_by(|a, b| {
                let cmp = match field {
                    ...
                    "original_path" => a.original_path.as_deref().cmp(&b.original_path.as_deref()),
                    "size" => a.size.cmp(&b.size),
                    "modified" => a.modified.cmp(&b.modified),
                    "trashed_at" => cmp_optional_utc(a.trashed_at, b.trashed_at),
    ...
    async fn read_range(&self, path: &str, start: u64, end: u64)
    ...
    fn get_recursive_size(&self, path: &str)
    ...
    async fn submit_batch_compress(
        ...
        _options: crate::utils::CompressionOptions,
        _delete_source: bool,
    ) -> VfsResult<String> {
        Err(crate::vfs::error::VfsError::Internal(
            "Persistent tasks not supported by this storage backend".to_string(),
    ...
    async fn submit_batch_decompress(
        ...
        _paths: Vec<String>,
        _output_dir: String,
        _options: crate::utils::DecompressionOptions,
        _delete_archive: bool,
    ) -> VfsResult<String> {
        Err(crate::vfs::error::VfsError::Internal(
            "Persistent tasks not supported by this storage backend".to_string(),
        ))
