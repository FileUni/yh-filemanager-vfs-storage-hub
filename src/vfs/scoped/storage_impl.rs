...
impl VfsStorage for ScopedVfsStorageEngine {
    ...
    async fn compress(&self, source_path: &str, target_path: &str, user_id: &str, password: Option<&str>, encrypt_filenames: bool) -> VfsResult<VfsFileInfo> {
        crate::utils::compression::compress_compat(self.pool.as_ref(), source_path, target_path, user_id, password, encrypt_filenames).await
    }
    async fn decompress(&self, archive_path: &str, target_dir: &str, user_id: &str, overwrite: bool, password: Option<&str>) -> VfsResult<VfsFileInfo> {
        crate::utils::compression::decompress_compat(self.pool.as_ref(), archive_path, target_dir, user_id, overwrite, password).await
    }
    async fn create_temp_file_for_upload(&self) -> VfsResult<(std::path::PathBuf, VfsTempFileGuard)> {
        self.temp_manager.create_user_temp_file(&self.user_id, "upload").await.map_err(|e| VfsError::Internal(e.to_string()))
    }
    ...
    async fn list_favorites_paginated(&self, params: VfsPaginationParams<'_>, color_filter: Option<i32>) -> VfsResult<(Vec<VfsFileInfo>, i64)> {
        self.list_favorites_paginated_impl(params, color_filter).await
    }
    ...
    async fn migrate_storage(&self, _path: &str, _target_storage_id: &str) -> VfsResult<VfsFileInfo> {
        Err(VfsError::Internal("Not implemented".to_string()))
    }
    async fn submit_batch_move(&self, src_paths: Vec<String>, dst_dir: String) -> VfsResult<String> {
        self.submit_batch_move_impl(src_paths, dst_dir).await
    }
    async fn submit_batch_copy(&self, src_paths: Vec<String>, dst_dir: String) -> VfsResult<String> {
    ...
    async fn submit_batch_compress(&self, paths: Vec<String>, archive_name: String, options: crate::utils::CompressionOptions, delete_source: bool) -> VfsResult<String> {
        self.submit_batch_compress_impl(paths, archive_name, options, delete_source).await
    }
    async fn submit_batch_decompress(&self, paths: Vec<String>, output_dir: String, options: crate::utils::DecompressionOptions, delete_archive: bool) -> VfsResult<String> {
        self.submit_batch_decompress_impl(paths, output_dir, options, delete_archive).await
    }
}
