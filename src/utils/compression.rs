...
pub async fn decompress_task(
    ...
    let cfg = crate::config::get_vfs_hub_config().await;
    let fc = cfg.get_file_compress();
    let allowed = yh_config_infra::config_require!(
        fc.decompression_formats,
        "vfs_storage_hub",
        "file_compress.decompression_formats"
    );
    if !is_extension_allowed(src_path, allowed) {
        return Err(anyhow::anyhow!(
            "Decompression format not supported for: {}",
            src_path
        ));
    }
    engine
        ...
        .map_err(|e| anyhow::anyhow!("Quota check failed: {}", e))?;
    let info = engine.stat(src_path).await?;
    let max_archive_size_mb = yh_config_infra::config_require_clone!(
    ...
    if info.size > max_archive_size {
        return Err(anyhow::anyhow!(
            "Decompression aborted: archive size exceeds limit"
        ));
    }
    let manager =
        yh_config_infra::config_require_manager!(get_global_manager(), "external_process_manager");
    manager
        .run_with_permit(
            yh_external_process_manager::TaskPriority::Normal,
            || async move {
                ...
                if use_7z {
                    ...
                    let mut child = Command::new(if exe_path.is_empty() { "7z" } else { exe_path })
                        ...
                        .arg(format!("-mmt={}", fc.get_effective_max_cpu_threads()))
                        .arg("-y")
                        .args(if let Some(pwd) = &opts.password {
                            vec![format!("-p{}", pwd)]
                        } else {
                            vec![]
                        })
                        .args(if opts.overwrite {
                            vec!["-aoa"]
                        } else {
                            vec!["-aos"]
                        })
                    ...
                    let timeout_duration =
                        Duration::from_secs(yh_config_infra::config_require_clone!(
                            fc.timeout_secs,
                            "vfs_storage_hub",
                    ...
                    match tokio::time::timeout(timeout_duration, child.wait()).await {
                        Ok(Ok(status)) => {
                            if !status.success() {
                                return Err(anyhow::anyhow!(
                                    "7z extraction failed with status {}",
                                    status
                                ));
                            }
                        }
                    ...
                    let gzip_output_name = if format == CompressionFormat::Gzip {
                        Some(
                            Path::new(src_path)
                                ...
                                .ok_or_else(|| {
                                    anyhow::anyhow!(
                                        "Invalid gzip source path without file stem: {}",
                                        src_path
                                    )
                                })?
                                .to_os_string(),
                    ...
                    tokio::task::spawn_blocking(move || match format {
                        CompressionFormat::Zip => {
                            decompress_zip_native(&archive_path, &output_dir, password.as_deref())
                        }
                        CompressionFormat::TarGz => {
                            decompress_tar_gz_native(&archive_path, &output_dir)
                        }
                        CompressionFormat::Gzip => {
                            let file_name = gzip_output_name.ok_or_else(|| {
                                anyhow::anyhow!(
                                    "Missing derived output file name for gzip decompression"
                                )
                            })?;
                        ...
                        _ => Err(anyhow::anyhow!(
                            "Native decompression for {:?} is not supported without 7z.",
                            format
                        )),
                    })
                    .await??;
                ...
                let max_extract_size_gb = yh_config_infra::config_require_clone!(
...
pub async fn stream_compress_reader(
    engine: Arc<dyn VfsStorage>,
    src_path: String,
...
pub async fn list_archive_contents(
    ...
    let cfg = crate::config::get_vfs_hub_config().await;
    let fc = cfg.get_file_compress();
    if !yh_config_infra::config_require_clone!(
    ...
    if !is_extension_allowed(archive_path, allowed) {
        return Err(anyhow::anyhow!(
            "Archive browsing not supported for format: {}",
            archive_path
        ));
    }
    let exe_path = fc.get_exe_7zip_path();
...
pub async fn list_archive_content(
    engine: Arc<dyn VfsStorage>,
    src_path: String,
    _user_id: String,
) -> Result<Vec<ArchiveEntry>, anyhow::Error> {
...
pub async fn compress_compat(
    ...
    let format = if let Some(parsed) = CompressionFormat::from_path(target_path) {
        ...
        yh_console_log::yhlog(
            "warn",
            &format!(
                "Unknown target archive extension for '{}', fallback to zip format",
                target_path
            ),
        );
        CompressionFormat::Zip
    ...
    let opts = CompressionOptions {
        ...
        encrypt_filenames,
        compression_level: yh_config_infra::config_require_clone!(
            fc.compression_max_level,
            "vfs_storage_hub",
            "file_compress.compression_max_level"
        ),
    };
    compress_task(engine, source_path, target_path, user_id, &opts, false)
        .await
        .map_err(|e| VfsError::Internal(e.to_string()))
}
pub async fn decompress_compat(
    engine: &dyn VfsStorage,
    archive_path: &str,
    target_dir: &str,
    user_id: &str,
    ...
    decompress_task(engine, archive_path, target_dir, user_id, &opts, false)
        .await
        .map_err(|e| VfsError::Internal(e.to_string()))
}
#[cfg(test)]
mod tests {
    ...
    fn test_compression_format_from_path() {
        ...
        assert_eq!(
            CompressionFormat::from_path("test.7z"),
            Some(CompressionFormat::SevenZip)
        );
        assert_eq!(
    ...
    fn test_compression_format_from_path_case_insensitive() {
        assert_eq!(
            CompressionFormat::from_path("FILE.ZIP"),
            Some(CompressionFormat::Zip)
        );
        assert_eq!(
            CompressionFormat::from_path("FILE.7Z"),
            Some(CompressionFormat::SevenZip)
        );
        assert_eq!(
            CompressionFormat::from_path("FILE.TAR"),
            Some(CompressionFormat::Tar)
        );
        assert_eq!(
            CompressionFormat::from_path("FILE.GZ"),
            Some(CompressionFormat::Gzip)
        );
        assert_eq!(
            CompressionFormat::from_path("FILE.TAR.GZ"),
            Some(CompressionFormat::TarGz)
        );
    }
    ...
    fn test_compression_format_from_path_with_directory() {
        assert_eq!(
            CompressionFormat::from_path("/path/to/archive.zip"),
            Some(CompressionFormat::Zip)
        );
        assert_eq!(
            CompressionFormat::from_path("C:\\Users\\test\\file.7z"),
            Some(CompressionFormat::SevenZip)
        );
        assert_eq!(
            CompressionFormat::from_path("/data/backup.tar.gz"),
            Some(CompressionFormat::TarGz)
        );
    }
    ...
    fn test_zip_integrity_native() -> anyhow::Result<()> {
        ...
        // Verify contents
        assert_eq!(
            std::fs::read_to_string(out_dir.join("file1.txt"))?,
            file1_content
        );
        assert_eq!(
            std::fs::read_to_string(out_dir.join("subdir/file2.txt"))?,
            file2_content
        );
        // Cleanup
